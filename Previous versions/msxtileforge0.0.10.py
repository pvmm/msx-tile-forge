...
MSX2_RGB7_VALUES = [
    (0, 0, 0), # 0 Transparent (treated as Black)
    (0, 0, 0), # 1 Black
...
# (Indices for defaults like BLACK_IDX remain the same)
BLACK_IDX = 1
MED_GREEN_IDX = 2
WHITE_IDX = 15
...
class TileEditorApp:
    def __init__(self, root):
        ...
        # --- Dynamic Palette ---
        self.active_msx_palette = [] # Initialize as empty list
        # Loop through the standard MSX1 RGB values
        for r, g, b in MSX2_RGB7_VALUES:
            # Convert each to its canonical hex representation using the app's method
            canonical_hex = self._rgb7_to_hex(r, g, b) # Call the instance method
    ...
    def create_palette_editor_widgets(self, parent_frame): # New Method
        ...
        reset_palette_button.pack(pady=(0, 5), fill="x") # Place below RGB frame
    ...
    def create_tile_editor_widgets(self, parent_frame):
        # ... (Setup for main_frame, left_frame, editor_frame, attr_frame as before) ...
        main_frame = ttk.Frame(parent_frame)
        main_frame.pack(expand=True, fill="both")
        ...
        # --- vvv ADD SHIFT BUTTONS vvv ---
        # Row 1: Shift/Circular Rotate
        shift_up_button = ttk.Button(transform_frame, text="Shift Up", command=self.shift_tile_up) # Changed command name
        shift_up_button.grid(row=1, column=0, padx=3, pady=3)

        shift_down_button = ttk.Button(transform_frame, text="Shift Down", command=self.shift_tile_down) # Changed command name
        shift_down_button.grid(row=1, column=1, padx=3, pady=3)

        shift_left_button = ttk.Button(transform_frame, text="Shift Left", command=self.shift_tile_left) # Changed command name
        shift_left_button.grid(row=1, column=2, padx=3, pady=3)

        shift_right_button = ttk.Button(transform_frame, text="Shift Right", command=self.shift_tile_right) # Changed command name
        shift_right_button.grid(row=1, column=3, padx=3, pady=3) # Add column 3
        # --- ^^^ ADD SHIFT BUTTONS ^^^ ---

        # ... (Rest of right_frame setup as before) ...
        ...
        palette_frame = ttk.LabelFrame(right_frame, text="Color Selector (Click to draw)"); palette_frame.grid(row=0, column=0, pady=(0, 10), sticky=(tk.N, tk.W, tk.E))
        self.tile_editor_palette_canvas = tk.Canvas(palette_frame, width=4 * (PALETTE_SQUARE_SIZE + 2), height=4 * (PALETTE_SQUARE_SIZE + 2), borderwidth=0, highlightthickness=0); self.tile_editor_palette_canvas.grid(row=0, column=0); self.tile_editor_palette_canvas.bind("<Button-1>", self.handle_tile_editor_palette_click)
        ...
        viewer_canvas_width = NUM_TILES_ACROSS * (VIEWER_TILE_SIZE + 1) + 1; num_rows_in_viewer = math.ceil(MAX_TILES / NUM_TILES_ACROSS); viewer_canvas_height = num_rows_in_viewer * (VIEWER_TILE_SIZE + 1) + 1; viewer_hbar = ttk.Scrollbar(viewer_frame, orient=tk.HORIZONTAL); viewer_vbar = ttk.Scrollbar(viewer_frame, orient=tk.VERTICAL)
        self.tileset_canvas = tk.Canvas( viewer_frame, bg="lightgrey", scrollregion=(0, 0, viewer_canvas_width, viewer_canvas_height), xscrollcommand=viewer_hbar.set, yscrollcommand=viewer_vbar.set); viewer_hbar.config(command=self.tileset_canvas.xview); viewer_vbar.config(command=self.tileset_canvas.yview)
        self.tileset_canvas.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.W, tk.E)); viewer_vbar.grid(row=0, column=1, sticky=(tk.N, tk.S)); viewer_hbar.grid(row=1, column=0, sticky=(tk.W, tk.E)); viewer_frame.grid_rowconfigure(0, weight=1); viewer_frame.grid_columnconfigure(0, weight=1); self.tileset_canvas.bind("<Button-1>", self.handle_tileset_click)
        add_tile_button = ttk.Button(right_frame, text="Add New Tile", command=self.add_new_tile); add_tile_button.grid(row=2, column=0, sticky=tk.W, pady=(5, 0))
        self.tile_info_label = ttk.Label(right_frame, text="Tile: 0/0"); self.tile_info_label.grid(row=3, column=0, sticky=tk.W, pady=(2,0))
    ...
    # --- Tile Transformation Methods ---

    def flip_tile_horizontal(self):
        """Flips the current tile pattern horizontally."""
        global tileset_patterns, current_tile_index, num_tiles_in_set

        # Validate current tile index
        if not (0 <= current_tile_index < num_tiles_in_set):
            messagebox.showwarning("Flip Tile", "No valid tile selected to flip.")
            return

        # Get current pattern
        current_pattern = tileset_patterns[current_tile_index]

        # Create a new pattern list to store the flipped result
        new_pattern = [[0] * TILE_WIDTH for _ in range(TILE_HEIGHT)]

        # Iterate through each row
        for r in range(TILE_HEIGHT):
            # Get the original row
            original_row = current_pattern[r]
            # Reverse the row and assign it to the new pattern
            new_pattern[r] = original_row[::-1] # Slice notation to reverse list

        # Replace the old pattern with the new flipped pattern
        tileset_patterns[current_tile_index] = new_pattern

        # Invalidate cache and update display
        # Color attributes remain the same for horizontal flip
        self.invalidate_tile_cache(current_tile_index)
        self.update_all_displays(changed_level="tile")
        print(f"Tile {current_tile_index} flipped horizontally.")

    def flip_tile_vertical(self):
        """Flips the current tile pattern and its row colors vertically."""
        global tileset_patterns, tileset_colors, current_tile_index, num_tiles_in_set
        ...
        # Reverse the order of rows in the pattern list IN PLACE
        tileset_patterns[current_tile_index].reverse()

        # Reverse the order of row color attributes IN PLACE
        tileset_colors[current_tile_index].reverse()

        # Invalidate cache and update display
        self.invalidate_tile_cache(current_tile_index)
        self.update_all_displays(changed_level="tile")
        print(f"Tile {current_tile_index} flipped vertically (pattern and colors).")

    def rotate_tile_90cw(self):
        """Rotates the current tile pattern 90 degrees clockwise. Resets row colors."""
        global tileset_patterns, tileset_colors, current_tile_index, num_tiles_in_set
        global WHITE_IDX, BLACK_IDX # Need default color indices

        # Validate current tile index
        if not (0 <= current_tile_index < num_tiles_in_set):
            messagebox.showwarning("Rotate Tile", "No valid tile selected to rotate.")
            return

        # Get current pattern
        current_pattern = tileset_patterns[current_tile_index]

        # Create a new pattern list for the rotated result, initialized with 0s
        new_pattern = [[0 for _ in range(TILE_WIDTH)] for _ in range(TILE_HEIGHT)]

        # Perform the 90-degree clockwise rotation
        # New row 'nr' corresponds to old column 'c'
        # New col 'nc' corresponds to TILE_HEIGHT - 1 - old row 'r'
        for r in range(TILE_HEIGHT):
            for c in range(TILE_WIDTH):
                new_row_index = c
                new_col_index = (TILE_HEIGHT - 1) - r
                new_pattern[new_row_index][new_col_index] = current_pattern[r][c]

        # Replace the old pattern
        tileset_patterns[current_tile_index] = new_pattern

        # --- Reset row colors to default (White on Black) ---
        # Because rotation makes original row colors meaningless
        default_colors = [(WHITE_IDX, BLACK_IDX) for _ in range(TILE_HEIGHT)]
        tileset_colors[current_tile_index] = default_colors

        # Inform user about color reset
        messagebox.showinfo("Rotation Complete", "Tile rotated clockwise.\nRow colors have been reset to default (White/Black).")
    ...
    # --- Tile Shifting/Circular Rotate Methods ---

    def shift_tile_up(self):
        """Shifts the current tile pattern and colors UP by one row (wraps around)."""
        global tileset_patterns, tileset_colors, current_tile_index, num_tiles_in_set

        if not (0 <= current_tile_index < num_tiles_in_set):
            messagebox.showwarning("Shift Tile", "No valid tile selected.")
            return

        # Get references to the current tile's data lists
        current_pattern = tileset_patterns[current_tile_index]
        current_colors = tileset_colors[current_tile_index]

        # --- Perform Circular Shift Up ---
        # Store the first element (top row)
        first_pattern_row = current_pattern[0]
        first_color_row = current_colors[0]
        # Shift elements from index 1 upwards
    ...
    def shift_tile_down(self):
        """Shifts the current tile pattern and colors DOWN by one row (wraps around)."""
        global tileset_patterns, tileset_colors, current_tile_index, num_tiles_in_set
        ...
        # --- Perform Circular Shift Down ---
        # Store the last element (bottom row)
        last_pattern_row = current_pattern[TILE_HEIGHT - 1]
        last_color_row = current_colors[TILE_HEIGHT - 1]
        # Shift elements from index TILE_HEIGHT - 2 downwards (iterate backwards)
    ...
    def shift_tile_left(self):
        """Shifts the current tile pattern LEFT by one column (wraps around)."""
        global tileset_patterns, current_tile_index, num_tiles_in_set
        ...
        # Iterate through each row to perform the shift
        for r in range(TILE_HEIGHT):
            row_data = current_pattern[r] # Get reference to the row list

            # --- Perform Circular Shift Left on the row ---
            if TILE_WIDTH > 0: # Avoid error on empty row (shouldn't happen)
        ...
        # Colors are per-row, so they are unaffected by horizontal shift
        self.invalidate_tile_cache(current_tile_index)
        self.update_all_displays(changed_level="tile")
        print(f"Tile {current_tile_index} shifted left.")

    def shift_tile_right(self):
        """Shifts the current tile pattern RIGHT by one column (wraps around)."""
        global tileset_patterns, current_tile_index, num_tiles_in_set
        ...
        for r in range(TILE_HEIGHT):
            ...
            # --- Perform Circular Shift Right on the row ---
            if TILE_WIDTH > 0:
        ...
        # Colors are per-row, unaffected by horizontal shift
        self.invalidate_tile_cache(current_tile_index)
        self.update_all_displays(changed_level="tile")
        print(f"Tile {current_tile_index} shifted right.")

    def reset_palette_to_default(self):
        """Resets the active palette to the default MSX2 values."""
        ...
        if confirm:
            # Rebuild the default palette using the stored RGB values
            new_default_palette = []
            for r, g, b in MSX2_RGB7_VALUES: # Use the MSX2 defaults defined globally
                canonical_hex = self._rgb7_to_hex(r, g, b)
                new_default_palette.append(canonical_hex)

            # Check if the palette actually changed before proceeding
            if self.active_msx_palette != new_default_palette:
                # Update the active palette
                self.active_msx_palette = new_default_palette
                # Reset selections in the palette editor
                self.selected_palette_slot = 0
                global selected_color_index # Need global for tile editor palette
                selected_color_index = 0

                # --- Crucial: Palette changed, update everything ---
                self.clear_all_caches() # Clear rendered images
                self.update_all_displays(changed_level="all") # Redraw all tabs
