...
class TileEditorApp:
    ...
    # --- Image Generation --- (Identical to previous version)
    ...
    def create_supertile_image(self, supertile_index, total_size):
        ...
        for y in range(total_size):
            ...
            for x in range(total_size):
                ...
                tile_idx = definition[mini_tile_r][mini_tile_c]; pixel_color = INVALID_TILE_COLOR
                if 0 <= tile_idx < num_tiles_in_set:
                    tile_r = min(TILE_HEIGHT - 1, int(y_in_mini * mini_tile_pixel_h)); tile_c = min(TILE_WIDTH - 1, int(x_in_mini * mini_tile_pixel_w))
                    try: pattern = tileset_patterns[tile_idx]; colors = tileset_colors[tile_idx]; fg_idx, bg_idx = colors[tile_r]
                         pixel_val = pattern[tile_r][tile_c]; pixel_color = MSX_COLORS[fg_idx] if pixel_val == 1 else MSX_COLORS[bg_idx]
                    except IndexError: print(f"Warning: IndexError T:{tile_idx} P:[{tile_r},{tile_c}]"); pixel_color = INVALID_TILE_COLOR
                row_colors.append(pixel_color)
            try: img.put("{" + " ".join(row_colors) + "}", to=(0, y))
            except tk.TclError as e: print(f"Warning: TclError ST {supertile_index} size {total_size} row {y}: {e}"); img.put(row_colors[0], to=(0, y, total_size, y+1))
        self.supertile_image_cache[cache_key] = img; return img


    # --- Menu Creation --- (Identical)
    def create_menu(self):
        ...
        file_menu.add_command(label="New Project (All)", command=self.new_project); file_menu.add_separator()
        file_menu.add_command(label="Open Tileset (.SC4Tiles)...", command=self.open_tileset); file_menu.add_command(label="Save Tileset (.SC4Tiles)...", command=self.save_tileset); file_menu.add_separator()
        file_menu.add_command(label="Open Supertiles (.SC4Super)...", command=self.open_supertiles); file_menu.add_command(label="Save Supertiles (.SC4Super)...", command=self.save_supertiles); file_menu.add_separator()
        file_menu.add_command(label="Open Map (.SC4Map)...", command=self.open_map); file_menu.add_command(label="Save Map (.SC4Map)...", command=self.save_map); file_menu.add_separator(); file_menu.add_command(label="Exit", command=self.root.quit)
        edit_menu = tk.Menu(menubar, tearoff=0); menubar.add_cascade(label="Edit", menu=edit_menu)
        edit_menu.add_command(label="Set Tileset Size...", command=self.set_tileset_size); edit_menu.add_command(label="Set Supertile Count...", command=self.set_supertile_count); edit_menu.add_command(label="Set Map Dimensions...", command=self.set_map_dimensions); edit_menu.add_separator()
        edit_menu.add_command(label="Clear Current Tile", command=self.clear_current_tile); edit_menu.add_command(label="Clear Current Supertile", command=self.clear_current_supertile); edit_menu.add_command(label="Clear Map", command=self.clear_map)
    ...
    def create_tile_editor_widgets(self, parent_frame):
        ...
        self.tile_info_label.grid(row=3, column=0, sticky=tk.W, pady=(2,0)) # Adjusted row

    def create_supertile_editor_widgets(self, parent_frame):
        main_frame = ttk.Frame(parent_frame); main_frame.pack(expand=True, fill="both")
        left_frame = ttk.Frame(main_frame); left_frame.grid(row=0, column=0, sticky=tk.N, padx=(0, 10))
        # ... Supertile Definition Canvas setup ...
        ...
        # --- vvv ADDED SUPERTILE BUTTON vvv ---
        ...
        # --- ^^^ ADDED SUPERTILE BUTTON ^^^ ---

        self.supertile_sel_info_label = ttk.Label(right_frame, text=f"Supertiles: {num_supertiles}")
        self.supertile_sel_info_label.grid(row=3, column=0, sticky=tk.W, pady=(2,0)) # Adjusted row

    def create_map_editor_widgets(self, parent_frame):
        # ... (Map editor widget creation remains the same) ...
        main_frame = ttk.Frame(parent_frame); main_frame.pack(expand=True, fill="both")
        ...
        map_controls_frame = ttk.Frame(left_frame); map_controls_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=(0, 5)); ttk.Label(map_controls_frame, text="Map Size:").grid(row=0, column=0, padx=(0,5)); self.map_size_label = ttk.Label(map_controls_frame, text=f"{map_width} x {map_height}"); self.map_size_label.grid(row=0, column=1)
        ...
        map_hbar = ttk.Scrollbar(map_canvas_frame, orient=tk.HORIZONTAL); map_vbar = ttk.Scrollbar(map_canvas_frame, orient=tk.VERTICAL); map_canvas_width = map_width * MAP_CELL_PREVIEW_SIZE; map_canvas_height = map_height * MAP_CELL_PREVIEW_SIZE
        ...
        map_hbar.config(command=self.map_canvas.xview); map_vbar.config(command=self.map_canvas.yview); self.map_canvas.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.W, tk.E)); map_vbar.grid(row=0, column=1, sticky=(tk.N, tk.S)); map_hbar.grid(row=1, column=0, sticky=(tk.W, tk.E)); map_canvas_frame.grid_rowconfigure(0, weight=1); map_canvas_frame.grid_columnconfigure(0, weight=1); self.map_canvas.bind("<Button-1>", self.handle_map_click); self.map_canvas.bind("<B1-Motion>", self.handle_map_drag)
        right_frame = ttk.Frame(main_frame); right_frame.grid(row=0, column=1, sticky=(tk.N, tk.S)); st_selector_frame = ttk.LabelFrame(right_frame, text="Supertile Palette (Click to select for map)"); st_selector_frame.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.W, tk.E)); right_frame.grid_rowconfigure(0, weight=1); right_frame.grid_columnconfigure(0, weight=1)
        st_sel_canvas_width = NUM_SUPERTILES_ACROSS * (SUPERTILE_SELECTOR_PREVIEW_SIZE + 1) + 1; st_sel_num_rows = math.ceil(MAX_SUPERTILES / NUM_SUPERTILES_ACROSS); st_sel_canvas_height = st_sel_num_rows * (SUPERTILE_SELECTOR_PREVIEW_SIZE + 1) + 1
    ...
    # --- Drawing Functions --- (Unchanged from PhotoImage version)
    ...
    def draw_editor_canvas(self): # Direct drawing
        ...
        for r in range(TILE_HEIGHT): fg_idx, bg_idx = colors[r]; fg_color = MSX_COLORS[fg_idx]; bg_color = MSX_COLORS[bg_idx]
            for c in range(TILE_WIDTH): color = fg_color if pattern[r][c] == 1 else bg_color; x1,y1 = c*EDITOR_PIXEL_SIZE, r*EDITOR_PIXEL_SIZE; x2,y2 = x1+EDITOR_PIXEL_SIZE, y1+EDITOR_PIXEL_SIZE; self.editor_canvas.create_rectangle(x1,y1,x2,y2,fill=color,outline="darkgrey",width=1)
    def draw_attribute_editor(self): # Direct labels
        if not (0 <= current_tile_index < num_tiles_in_set): return
    ...
    def draw_palette(self): # Direct items
    ...
    def draw_tileset_viewer(self, canvas, highlighted_tile_index): # Uses cache
    ...
    def update_tile_info_label(self): self.tile_info_label.config(text=f"Tile: {current_tile_index}/{num_tiles_in_set-1}")
    def draw_supertile_definition_canvas(self): # Uses cache
    ...
    def draw_supertile_selector(self, canvas, highlighted_supertile_index): # Uses cache
    ...
    def update_supertile_info_labels(self): self.supertile_def_info_label.config(text=f"Editing Supertile: {current_supertile_index}/{num_supertiles-1}"); self.supertile_tile_select_label.config(text=f"Selected Tile for Placing: {selected_tile_for_supertile}"); self.supertile_sel_info_label.config(text=f"Supertiles: {num_supertiles}")
    def draw_map_canvas(self): # Uses cache
        canvas = self.map_canvas; canvas.delete("all"); size = MAP_CELL_PREVIEW_SIZE
        map_canvas_width = map_width * size; map_canvas_height = map_height * size; str_scroll = f"0 0 {float(map_canvas_width)} {float(map_canvas_height)}"
        if canvas.cget("scrollregion") != str_scroll: canvas.config(scrollregion=(0, 0, map_canvas_width, map_canvas_height))
        for r in range(map_height):
            for c in range(map_width):
                 supertile_idx = map_data[r][c]; base_x = c * size; base_y = r * size
                 img = self.create_supertile_image(supertile_idx, size); canvas.create_image(base_x, base_y, image=img, anchor=tk.NW, tags=f"map_cell_{r}_{c}")
    ...
    # --- Event Handlers --- (Unchanged from previous version w/ cache invalidation)
    ...
    def handle_palette_click(self, event):
        ...
        item = self.palette_canvas.find_closest(event.x, event.y)[0]; tags = self.palette_canvas.gettags(item)
        for tag in tags:
            if tag.startswith("pal_"):
                try: new_index = int(tag.split("_")[1])
                     if new_index != selected_color_index: selected_color_index = new_index; self.draw_palette()
                     break
    ...
    def _paint_map_cell(self, event_x, event_y): # Direct redraw of cell
        global map_data, last_painted_map_cell
        canvas = self.map_canvas; size = MAP_CELL_PREVIEW_SIZE; c = int(canvas.canvasx(event_x) // size); r = int(canvas.canvasy(event_y) // size)
        if 0 <= r < map_height and 0 <= c < map_width:
            if (r, c) != last_painted_map_cell:
                 if map_data[r][c] != selected_supertile_for_map:
                    map_data[r][c] = selected_supertile_for_map; base_x = c * size; base_y = r * size
                    img = self.create_supertile_image(selected_supertile_for_map, size); canvas.delete(f"map_cell_{r}_{c}")
    ...
    # --- File Menu Commands --- (Unchanged from previous version w/ cache handling)
        ...
        try:
            with open(filepath, 'rb') as f:
                 ...
                 for i in range(loaded_num_tiles):
                     for r in range(TILE_HEIGHT): byte_val = struct.unpack('B', f.read(1))[0]; [ new_patterns[i][r].__setitem__(c, (byte_val >> (7 - c)) & 1) for c in range(TILE_WIDTH) ] # Compact form
    ...
    def save_supertiles(self):
        ...
        try:
            with open(filepath, 'wb') as f:
                ...
                for i in range(num_supertiles): [ f.write(struct.pack('B', supertiles_data[i][r][c])) for r in range(SUPERTILE_GRID_DIM) for c in range(SUPERTILE_GRID_DIM) ] # Compact
    ...
    def open_supertiles(self):
        ...
        try:
            with open(filepath, 'rb') as f:
                 ...
                 for i in range(loaded_num_st): [ new_st_data[i][r].__setitem__(c, struct.unpack('B', f.read(1))[0]) for r in range(SUPERTILE_GRID_DIM) for c in range(SUPERTILE_GRID_DIM) ] # Compact
    ...
    def save_map(self):
        ...
        if not filepath: return
        try:
            with open(filepath, 'wb') as f: f.write(struct.pack('>HH', map_width, map_height)); [ f.write(struct.pack('B', map_data[r][c])) for r in range(map_height) for c in range(map_width) ] # Compact
    ...
    def open_map(self):
        ...
        if not filepath: return
        try:
            with open(filepath, 'rb') as f: loaded_w, loaded_h = struct.unpack('>HH', f.read(4))
                 ...
                 for r in range(loaded_h): [ new_map_data[r].__setitem__(c, struct.unpack('B', f.read(1))[0]) for c in range(loaded_w) ] # Compact
    ...
    # --- Edit Menu Commands --- (Unchanged from previous version w/ cache handling)
    def set_tileset_size(self):
        ...
        if new_size_str: try: new_size = int(new_size_str)
             if 1 <= new_size <= MAX_TILES: reduced = new_size < num_tiles_in_set
                  if reduced and not messagebox.askokcancel("Reduce Size", "..."): return
                  if reduced: [ self.invalidate_tile_cache(i) for i in range(new_size, num_tiles_in_set) ]
                  num_tiles_in_set = new_size; current_tile_index = max(0, min(current_tile_index, num_tiles_in_set - 1)); selected_tile_for_supertile = max(0, min(selected_tile_for_supertile, num_tiles_in_set -1)) if num_tiles_in_set > 0 else 0
                  self.update_all_displays(changed_level="all")
    ...
    def set_supertile_count(self):
        ...
        if new_count_str: try: new_count = int(new_count_str)
             if 1 <= new_count <= MAX_SUPERTILES: reduced = new_count < num_supertiles
                  if reduced and not messagebox.askokcancel("Reduce Count", "..."): return
                  if reduced: [ self.invalidate_supertile_cache(i) for i in range(new_count, num_supertiles) ]
                  num_supertiles = new_count; current_supertile_index = max(0, min(current_supertile_index, num_supertiles - 1)); selected_supertile_for_map = max(0, min(selected_supertile_for_map, num_supertiles - 1)) if num_supertiles > 0 else 0
    ...
    def set_map_dimensions(self):
        ...
        if dims: try: parts = dims.lower().split('x'); new_w, new_h = int(parts[0].strip()), int(parts[1].strip())
             ...
             new_map_data = [[0]*new_w for _ in range(new_h)]; [ [ new_map_data[r].__setitem__(c, map_data[r][c]) for c in range(min(map_width, new_w)) ] for r in range(min(map_height, new_h)) ] # Compact
             map_width = new_w; map_height = new_h; map_data = new_map_data; self.update_all_displays(changed_level="map")
    ...
    def clear_current_tile(self):
        ...
        if not (0 <= current_tile_index < num_tiles_in_set): return
        if messagebox.askokcancel("Clear Tile", ...): tileset_patterns[current_tile_index] = [[0]*TILE_WIDTH for _ in range(TILE_HEIGHT)]; tileset_colors[current_tile_index] = [(15, 1) for _ in range(TILE_HEIGHT)]; self.invalidate_tile_cache(current_tile_index); self.update_all_displays(changed_level="tile")
    def clear_current_supertile(self):
        global supertiles_data, current_supertile_index
        if not (0 <= current_supertile_index < num_supertiles): return
        if messagebox.askokcancel("Clear Supertile", ...): supertiles_data[current_supertile_index] = [[0]*SUPERTILE_GRID_DIM for _ in range(SUPERTILE_GRID_DIM)]; self.invalidate_supertile_cache(current_supertile_index); self.update_all_displays(changed_level="supertile")
    def clear_map(self):
        global map_data, map_width, map_height
        if messagebox.askokcancel("Clear Map", ...): map_data = [[0]*map_width for _ in range(map_height)]; self.update_all_displays(changed_level="map")


    # --- vvv NEW METHODS vvv ---

    def add_new_tile(self):
        """Adds a new blank tile to the end of the tileset."""
    ...
    def add_new_supertile(self):
        """Adds a new blank supertile (all tile 0) to the end."""
        ...
        # No cache invalidation needed for adding a *new* blank supertile
        self.update_all_displays(changed_level="supertile")
    ...
    def scroll_viewers_to_tile(self, tile_index):
        ...
        padding = 1; tile_size = VIEWER_TILE_SIZE; items_per_row = NUM_TILES_ACROSS
        row, _ = divmod(tile_index, items_per_row); target_y = row * (tile_size + padding)
        # Scroll main viewer
        try:
            scroll_info = self.tileset_canvas.cget("scrollregion").split()
            total_height = float(scroll_info[3])
            if total_height > 0: self.tileset_canvas.yview_moveto(min(1.0, target_y / total_height))
        ...
        try:
            ...
            if total_height_st > 0: self.st_tileset_canvas.yview_moveto(min(1.0, target_y / total_height_st))
    ...
    def scroll_selectors_to_supertile(self, supertile_index):
        ...
        if supertile_index < 0: return
        padding = 1; size = SUPERTILE_SELECTOR_PREVIEW_SIZE; items_per_row = NUM_SUPERTILES_ACROSS
        row, _ = divmod(supertile_index, items_per_row); target_y = row * (size + padding)
        # Scroll ST tab selector
        try:
            scroll_info = self.supertile_selector_canvas.cget("scrollregion").split()
            total_height = float(scroll_info[3])
            if total_height > 0: self.supertile_selector_canvas.yview_moveto(min(1.0, target_y / total_height))
        except Exception as e: print(f"Error scrolling ST selector: {e}")
        # Scroll Map tab selector
        try:
            scroll_info_map = self.map_supertile_selector_canvas.cget("scrollregion").split()
            total_height_map = float(scroll_info_map[3])
            if total_height_map > 0: self.map_supertile_selector_canvas.yview_moveto(min(1.0, target_y / total_height_map))
        except Exception as e: print(f"Error scrolling Map selector: {e}")
